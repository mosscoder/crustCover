...
This R package was developed as a tool to quantify biocrust development and cover from photo data. The methods are an adaptation of Fischer et al. (2012) in which a point and shoot camera and filter were used to determine NDVI of biocrusts. They calibrated the reflectance of the visible red (VIS) and near-infrared bands (NIR) from the images with a color checker of known reflectance values. From these corrected reflectance data they then calculated NDVI.  
...
## Establishing Color Checker Cells for Calibration 

To establish the positions of the color checker cells, call the following function and store it as an object:
...
## Establishing Observation Area

To establish the observation area, call the following function and store it as an object:
```{r, eval=F}
pot <- pot()
```
The function will load a template image, and the user must click 16 times to circumscribe the observation area. Call the function again to correct errors.  
...
```{r, eval=F}
ndvi(chart = chart, pot = pot, threshold = 0.75)
```
...
```{r, eval=F}
ndvi(chart = chart, pot = pot, agg.fact = 1)
```
